...
3. `--auto-merge` 時: pr skill の**通行証プロトコル**（手順 2〜3。コマンド形は pr skill を正とする）に従い PR を作成 → `gh pr merge --squash --delete-branch` → local main を `--ff-only` で同期。指定なしなら PR 作成コマンドを案内し、ユーザーが GitHub 上で確認してから merge する
   - マーカーの touch / rm は**セッション側（skill を起動した消費リポジトリ）のルート**に対して行う（検問 hook はセッション側のマーカーだけを見る）。配信元リポジトリへ cd したままの相対パス操作は不可
4. merge 後の各プロジェクトへの反映は `/iskwyuki-claude-plugins:update-plugins`（marketplace update → plugin update → asset 同期 → commit 案内をワンステップ実行）を案内
