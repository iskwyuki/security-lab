...
プラグイン更新とまとめて行う場合は `/iskwyuki-claude-plugins:update-plugins` がワンステップで実行する（本 skill は cache が既に最新のとき同期だけを単体で行う）。
...
- 同期後は `git status -- .claude/` を見せ、**同期で書き込んだパスだけ**を明示 add（`git add .claude/<type>/<name>` を rsync の転送結果から列挙。type 単位の一括 add も不可）して `git commit -m "chore: iskwyuki-claude-plugins 同期"` を案内する。`git add .claude/` 一括は使わない（同期対象外の `.claude/memory/` やプロジェクト固有 skill の WIP を巻き込むため）
